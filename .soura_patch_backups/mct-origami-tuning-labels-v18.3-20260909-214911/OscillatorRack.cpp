...
OscillatorCard::OscillatorCard(OscillatorDisplay display,std::function<void(unsigned)> remove,
    ...
    engineBacked_=(display_.ordinal==1 && bool(parameterSetter_) && bool(parameterGetter_));
    if(engineBacked_) {
...
void OscillatorCard::resized() {
    ...
    octaveSlider_.setBounds(tuning.withX(tuning.getX()).withWidth(tuningCellWidth).reduced(3,0));
    semitoneSlider_.setBounds(tuning.withX(tuning.getX()+tuningCellWidth).withWidth(tuningCellWidth).reduced(3,0));
    fineSlider_.setBounds(tuning.withX(tuning.getX()+tuningCellWidth*2).withWidth(tuningCellWidth).reduced(3,0));
...
void OscillatorRack::addOscillator() {
    ...
    },parameterSetter_,parameterGetter_);
