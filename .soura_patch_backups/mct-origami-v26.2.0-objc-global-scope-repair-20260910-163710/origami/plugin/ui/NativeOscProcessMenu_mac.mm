...
//
// IMPORTANT: Cocoa must be parsed before JuceHeader.h enters this Objective-C++
// translation unit. JUCE exposes names such as juce::Point and juce::Component;
// Apple's legacy Carbon headers also declare global Point and Component symbols.
// Importing Cocoa after JUCE makes those SDK declarations ambiguous in this TU.
...
- (instancetype)init {
    self=[super init];
    if(self) selectedTag_=-1;
    return self;
}
- (void)chooseProcess:(id)sender { selectedTag_=[sender tag]; }
@end

NSString* toNS(const char* text) {
    return [NSString stringWithUTF8String:(text != nullptr ? text : "")];
}
NSMenuItem* makeItem(dsp::OscProcessType type,dsp::OscProcessType current,MCTOrigamiProcessMenuTarget* target) {
