...
void OscillatorCard::paintContent(juce::Graphics& g,juce::Rectangle<int> body) {
    ...
    auto previewArea=working;
    auto caption=previewArea.removeFromBottom(captionHeight);

    const int squareSize=juce::jmax(96,juce::jmin(previewArea.getWidth(),previewArea.getHeight()));
    auto preview=juce::Rectangle<int>(squareSize,squareSize).withCentre(previewArea.getCentre());

    graph(g,preview);
    text(g,"INIT  /  "+display_.source.toUpperCase(),caption,7.7f,Palette::muted(),juce::Justification::centred);
...
void OscillatorRack::layoutCards() {
    ...
    cardWidth_=juce::jlimit(300,350,(viewport_.getWidth()-72)/3);
