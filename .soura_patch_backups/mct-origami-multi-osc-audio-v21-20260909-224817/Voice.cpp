...
void Voice::reset() noexcept { for (auto& module : moduleOscillators_) for (auto& oscillator : module) oscillator.reset(); envelope_.reset(); filter_.reset(); active_ = releasing_ = false; velocity_ = 0; order_ = 0; }
...
float Voice::next(const dsp::Wavetable& table, float position, float sustain,
    ...
    if (count == 1) {
        oscillatorMix = moduleOscillators_[0][0].next(table, frequency_ * frequencyScale, sampleRate_, position);
    } else {
        for (unsigned i = 0; i < count; ++i) {
            ...
            oscillatorMix += moduleOscillators_[0][i].next(
