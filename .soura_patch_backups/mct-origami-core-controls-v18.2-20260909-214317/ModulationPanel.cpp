...
void ModulationPanel::paintContent(juce::Graphics& g,juce::Rectangle<int> body) {
    ...
    for(int i=0;i<5;++i) {
        ...
        text(g,"NONE",row.removeFromLeft(row.getWidth()/4).reduced(8,0),8.5f,Palette::muted());
        auto amount=row.removeFromLeft(row.getWidth()/3).reduced(7,0);
        g.setColour(Palette::borderSoft());g.fillRoundedRectangle(float(amount.getX()),float(amount.getCentreY()-1),float(amount.getWidth()),2.0f,1.0f);
        const int amountFill=juce::roundToInt(amount.getWidth()*.28f);
        g.setColour(Palette::accent().withAlpha(.42f));g.fillRoundedRectangle(float(amount.getX()),float(amount.getCentreY()-1),float(amountFill),2.0f,1.0f);
        text(g,"Unassigned",row.reduced(8,0),9,Palette::muted());
    }
}
