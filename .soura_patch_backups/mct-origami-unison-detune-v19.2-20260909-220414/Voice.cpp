...
float Voice::next(const dsp::Wavetable& table, float position, float sustain,
                  const dsp::LowPassCoefficients& filter, double frequencyScale) noexcept {
    if (!active_) return 0;
    if (!std::isfinite(frequencyScale) || frequencyScale <= 0.0) frequencyScale = 1.0;
    const float sample = oscillator_.next(table, frequency_ * frequencyScale, sampleRate_, position) * envelope_.next(sustain) * velocity_;
