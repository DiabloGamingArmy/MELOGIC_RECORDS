...
OscillatorRack::OscillatorRack(): Panel("OSCILLATORS") {
    ...
    left_.onClick=[this]{viewport_.setViewPosition(juce::jmax(0,viewport_.getViewPositionX()-cardWidth_-12),0);};
    right_.onClick=[this]{viewport_.setViewPosition(viewport_.getViewPositionX()+cardWidth_+12,0);};
...
void OscillatorRack::layoutCards() {
    ...
    cardWidth_=juce::jlimit(330,390,(viewport_.getWidth()-84-32)/3);
    const int height=juce::jmax(0,viewport_.getHeight()-12);
    int x=0;for(auto& card:cards_) {card->setBounds(x,0,cardWidth_,height);x+=cardWidth_+12;}
