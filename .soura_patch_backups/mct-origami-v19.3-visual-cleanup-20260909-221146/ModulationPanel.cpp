...
void ModulationPanel::paintContent(juce::Graphics& g,juce::Rectangle<int> body) {
    ...
    auto lfo=body.removeFromLeft(juce::roundToInt(available*.32));
    body.removeFromLeft(8);
    auto routing=body;
    ...
    text(g,tabs_[static_cast<std::size_t>(selected_)].getButtonText()+" / DISPLAY",graphCaption,9,Palette::muted());
    ...
    for(int i=0;i<5;++i) {
        auto row=routing.removeFromTop(rowHeight).reduced(0,1);
        g.setColour(Palette::border().withAlpha(.55f));
        g.drawHorizontalLine(row.getBottom(),float(row.getX()),float(row.getRight()));
        ...
        g.fillRect(amount.getX(),amount.getCentreY(),amount.getWidth(),2);
