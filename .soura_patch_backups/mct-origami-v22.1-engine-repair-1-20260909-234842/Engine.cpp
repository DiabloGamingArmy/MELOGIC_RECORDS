...
bool OrigamiEngine::process(float* const* output,unsigned channels,std::size_t sampleCount) noexcept {
    ...
    for(auto& module:modules) {
        ...
        module.wtPosition=std::clamp(module.wtPosition,0.0f,1.0f);
        module.waveform=module.wtPosition*3.0f;
    }
...
OscillatorModuleId OrigamiEngine::addOscillatorModule() noexcept {
    ...
    return state.wtPosition=std::clamp(state.waveform/3.0f,0.0f,1.0f);
    state.tableId=dsp::BuiltinWavetableId::BasicShapes;
    oscillatorModules_.add(s);
}
