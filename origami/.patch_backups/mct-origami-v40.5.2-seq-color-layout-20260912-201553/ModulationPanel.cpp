...
ModulationPanel::ModulationPanel(ParameterSetter setter,ParameterGetter getter,
    ...
    sequenceViewport_.setScrollOnDragEnabled(false);
    ...
    sequenceContent_.painter=[this](juce::Graphics& g) {
        ...
        for(std::size_t i=0;i<sequenceSteps_.size();++i) {
            ...
            g.setColour(accent.withAlpha(enabled?.34f:.045f)); g.fillRect(fill);
...
void ModulationPanel::timerCallback() {
    ...
    // LFO trace points are generated by the independent high-rate UI monitor.
    // The timer owns only aging/lifecycle.
...
void ModulationPanel::updateSourceHistory(float) {
    ...
    for(std::size_t i=0;i<4;++i) {
        if(selected_==static_cast<int>(3+i) && modulationUiTelemetry().synthActive &&
           !envCanvas_.isEmpty()) {
            // Visible LFO: independent 1920-Hz UI simulation (64 x 30 Hz).
            constexpr double visualMonitorRate=monitorRate*visualTraceSubsteps_;
            const auto& shape=lfoMseg_[i];
            for(int sub=0;sub<visualTraceSubsteps_;++sub) {
                samples[3+i]=sourceMonitorLfos_[i].next(lfoSettings(cached_,i),visualMonitorRate);
                const float phase=juce::jlimit(0.0f,1.0f,
                    static_cast<float>(sourceMonitorLfos_[i].phase()));
                if(lastLfoTracePhase_>=0.0f && phase+0.25f<lastLfoTracePhase_)
                    lfoTraceTail_.clear();
                lastLfoTracePhase_=phase;
                lfoTracePhase_=phase;
                lfoTraceTail_.push_back({
                    msegPixel({phase,msegValue(shape,phase),0.0f}),0.0f
                });
            }
