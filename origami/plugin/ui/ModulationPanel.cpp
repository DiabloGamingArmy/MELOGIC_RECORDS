...
ModulationPanel::ModulationPanel(ParameterSetter setter,ParameterGetter getter,
    ...
    rotary(*this,chaosAmount_,chaosAmountLabel_,"CHAOS");
    chaosAmount_.setRange(0.0,1.0,.001);chaosAmount_.setTextBoxStyle(juce::Slider::TextBoxRight,false,54,18);
    chaosAmount_.setTooltip("Lorenz rho: higher values expand and destabilize the attractor");
    rotary(*this,chaosFlow_,chaosFlowLabel_,"FLOW");
    chaosFlow_.setRange(0.0,1.0,.001);chaosFlow_.setTextBoxStyle(juce::Slider::TextBoxRight,false,54,18);
    ...
    sequenceContent_.painter=[this](juce::Graphics& g) {
        ...
        const std::size_t active=juce::jmin<std::size_t>(sequenceSteps_.size()-1,
            static_cast<std::size_t>(runtimeVisualization_.sourcePhases[11]*sequenceSteps_.size()));
        for(std::size_t i=0;i<sequenceSteps_.size();++i) {
            ...
            if(i==active && visualizationEnabled(VisualizationEffect::Sequencer)) {
        ...
        if(!visualizationEnabled(VisualizationEffect::Sequencer)) {
            const float x=runtimeVisualization_.sourcePhases[11]*bounds.getWidth();
...
void ModulationPanel::syncFromModel() {
    ...
    if(selected_==0) {
        ...
        if(!rate_.isMouseButtonDown()) rate_.setValue(cached_.chaos.rateHz,juce::dontSendNotification);
...
juce::Path ModulationPanel::envelopePath(const dsp::EnvelopeSettings& e,
    ...
    auto seg=[this,&p](juce::Point<float> a,juce::Point<float> b,float curve){
        // UI-only high-density curve tessellation. This changes no DSP state.
        constexpr int visualCurveSegments=192;
        for(int i=1;i<=visualCurveSegments;++i) {
            const float t=float(i)/float(visualCurveSegments),s=curveShape(t,curve);
...
void ModulationPanel::timerCallback() {
    ...
    // LFO trace points interpolate between authoritative audio-runtime phase
    // snapshots. The timer owns only their aging/lifecycle.
    if(selected_<3 || selected_>6 || !modulationUiTelemetry().synthActive) {
        lfoTraceTail_.clear();
    ...
    if(selected_<=2 && bindings_.envelopeTrace && !envCanvas_.isEmpty()){
        ...
        const auto envIndex=static_cast<std::size_t>(selected_);
        if(trace_.active){
            const auto& r=trace_.envelopes[envIndex];
            if(r.stage!=dsp::Envelope::Stage::Idle){
                if(trace_.order!=lastTraceOrder_){
                    traceTail_.clear();lastTraceOrder_=trace_.order;
                ...
                havePreviousVisualEnvelopeRuntime_[envIndex]=true;
                while(traceTail_.size()>visualTraceMaxPoints_) traceTail_.pop_front();
            } else {
                havePreviousVisualEnvelopeRuntime_[envIndex]=false;
            }
        } else {
...
bool ModulationPanel::visualizationEnabled(VisualizationEffect effect) const noexcept {
    return (visualizationMask_&visualizationBit(effect))!=0;
}
...
void ModulationPanel::paintContent(juce::Graphics& g,juce::Rectangle<int> body) {
    ...
    if(selected_==9) {
        ...
        if(!visualizationEnabled(VisualizationEffect::Chaos)) {
            const float x=graph.getX()+runtimeVisualization_.sourcePhases[9]*graph.getWidth();
            g.setColour(signalSourceColour().withAlpha(.96f));
            g.drawVerticalLine(juce::roundToInt(x),graph.getY(),graph.getBottom());
        }
        if(visualizationEnabled(VisualizationEffect::Chaos) && chaosViewportHistory_.size()>1) {
        ...
        if(trace_.active){
            ...
            if(r.stage!=dsp::Envelope::Stage::Idle){
                ...
                if(envCanvas_.contains(head)){
                    ...
                    if(!visualizationEnabled(VisualizationEffect::Env)) {
                        g.setColour(signalSourceColour().withAlpha(.96f));
    ...
    if(selected_>=3 && selected_<=6) {
        ...
        if(!visualizationEnabled(VisualizationEffect::Lfo)) {
            const float x=r.getX()+runtimeVisualization_.sourcePhases[static_cast<std::size_t>(selected_)]*r.getWidth();
            g.setColour(signalSourceColour().withAlpha(.96f));
            g.drawVerticalLine(juce::roundToInt(x),r.getY(),r.getBottom());
        } else if(!lfoTraceTail_.empty()) {
    ...
    if(selected_==7) {
        if(!visualizationEnabled(VisualizationEffect::Function)) {
            const float x=r.getX()+runtimeVisualization_.sourcePhases[7]*r.getWidth();
            g.setColour(signalSourceColour().withAlpha(.96f));
            g.drawVerticalLine(juce::roundToInt(x),r.getY(),r.getBottom());
            return;
        }
        ...
        if(!visualizationEnabled(VisualizationEffect::Random)) {
            const float x=r.getX()+runtimeVisualization_.sourcePhases[8]*r.getWidth();
            g.setColour(signalSourceColour().withAlpha(.96f));
            g.drawVerticalLine(juce::roundToInt(x),r.getY(),r.getBottom());
            return;
        }
        ...
        const auto effect=selected_==10 ? VisualizationEffect::Drift : VisualizationEffect::Sequencer;
        if(!visualizationEnabled(effect)) {
            const auto index=static_cast<std::size_t>(juce::jlimit(0,11,selected_));
            const float x=r.getX()+runtimeVisualization_.sourcePhases[index]*r.getWidth();
            g.setColour(signalSourceColour().withAlpha(.96f));
            g.drawVerticalLine(juce::roundToInt(x),r.getY(),r.getBottom());
            return;
        }
        const auto index=static_cast<std::size_t>(juce::jlimit(0,13,selected_));
        const auto& history=sourceHistory_[index];
        if(!history.empty()) {
            std::size_t i=0;
            for(float sample:history) {
                const float x=r.getX()+r.getWidth()*float(i++)/float(juce::jmax<std::size_t>(1,history.size()-1));
                const float y=r.getCentreY()-sample*r.getHeight()*.45f;
                if(i==1)p.startNewSubPath(x,y);else p.lineTo(x,y);
            }
        }
...
void ModulationPanel::updateSourceHistory(float) {
    ...
    if(bindings_.visualization)
        runtimeVisualization_=bindings_.visualization();
    if(bindings_.visualizationMask)
        visualizationMask_=bindings_.visualizationMask();
    ...
    if(!sourceTrace_.active) {
        ...
        randomViewportHistory_.clear();chaosViewportHistory_.clear();
    ...
    if(selected_>=3 && selected_<=6 && !envCanvas_.isEmpty()) {
        const float phase=juce::jlimit(0.0f,1.0f,runtimeVisualization_.sourcePhases[3+lfoIndex]);
        if(lastLfoTracePhase_>=0.0f && phase+0.25f<lastLfoTracePhase_)
            lfoTraceTail_.clear();
        const float from=lastLfoTracePhase_<0.0f ? phase : lastLfoTracePhase_;
        const auto& shape=lfoMseg_[lfoIndex];
        for(int sub=1;sub<=visualTraceSubsteps_;++sub) {
            const float alpha=float(sub)/float(visualTraceSubsteps_);
            const float anchored=from+(phase-from)*alpha;
            lfoTraceTail_.push_back({msegPixel({anchored,msegValue(shape,anchored),0.0f}),0.0f});
        }
        lastLfoTracePhase_=lfoTracePhase_=phase;
        while(lfoTraceTail_.size()>visualTraceMaxPoints_) lfoTraceTail_.pop_front();
    }
    ...
    for(std::size_t i=0;i<sourceHistory_.size();++i) {
        ...
        history.push_back(juce::jlimit(-1.0f,1.0f,samples[i]));
...
void ModulationPanel::paintSourceHistoryBackgrounds(juce::Graphics& g) {
    ...
    for(std::size_t i=0;i<tabs_.size();++i) {
        ...
        for(std::size_t h=0;h<count;++h) {
            const float magnitude=juce::jlimit(0.0f,1.0f,std::abs(history[h]));
