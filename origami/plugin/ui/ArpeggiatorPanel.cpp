...
namespace {
constexpr const char* directionLabels[]={"UP","DOWN","UP / DOWN","ORDER","RANDOM"};
...
ArpeggiatorPanel::ArpeggiatorPanel(Setter setter,Getter getter,RuntimeGetter runtimeGetter)
    : setter_(std::move(setter)),getter_(std::move(getter)),runtimeGetter_(std::move(runtimeGetter)) {
...
void ArpeggiatorPanel::configure() {
    ...
    enable_.setTooltip("Enable or bypass arpeggiator");
    latch_.setButtonText("LATCH");
    latch_.setClickingTogglesState(true);
    latch_.setTooltip("Keep the captured chord after keys are released");
    ...
    tempo_.setTooltip("Internal master tempo");
    configureKnob(gate_,"%",5.0,100.0,1.0,72.0);
    gate_.setTooltip("Note duration as a percentage of each step");
    configureKnob(swing_,"%",0.0,75.0,1.0,0.0);
    swing_.setTooltip("Alternating timing displacement");
...
void ArpeggiatorPanel::setDirection(int index) {
    ...
    s.direction=static_cast<mct::origami::ArpeggiatorState::Direction>(juce::jlimit(0,4,index));
    setter_(s);
    syncFromModel();
}
void ArpeggiatorPanel::setRate(int index) {
    if(syncing_ || !setter_) return;
    auto s=getter_?getter_():mct::origami::ArpeggiatorState{};
    s.rateIndex=juce::jlimit(0,6,index);
    setter_(s);
    syncFromModel();
}
void ArpeggiatorPanel::setOctaves(int octaves) {
    if(syncing_ || !setter_) return;
    auto s=getter_?getter_():mct::origami::ArpeggiatorState{};
    s.octaveSpan=juce::jlimit(1,4,octaves);
    setter_(s);
    syncFromModel();
}
...
void ArpeggiatorPanel::resized() {
    ...
    auto master=area.removeFromTop(72).reduced(12,10);
    enable_.setBounds(master.removeFromLeft(86).reduced(2,6));
    latch_.setBounds(master.removeFromLeft(92).reduced(2,6));
    master.removeFromLeft(18);

    auto clock=master.removeFromLeft(210);
    const int clockW=clock.getWidth()/2;
    clockButtons_[0].setBounds(clock.removeFromLeft(clockW).reduced(1,6));
    clockButtons_[1].setBounds(clock.reduced(1,6));

    master.removeFromLeft(16);
    tempo_.setBounds(master.removeFromLeft(92).reduced(8,-2));
    ...
    lower.removeFromLeft(14);
    auto timing=lower;

    auto p=pattern.reduced(16,42);
    auto directionRow=p.removeFromTop(36);
    ...
    for(std::size_t i=0;i<directionButtons_.size();++i) {
        ...
        directionButtons_[i].setBounds(cell.reduced(1,3));
    }

    p.removeFromTop(28);
    auto rateRow=p.removeFromTop(36);
    ...
    for(std::size_t i=0;i<rateButtons_.size();++i) {
        ...
        rateButtons_[i].setBounds(cell.reduced(1,3));
    }

    p.removeFromTop(28);
    auto octaveRow=p.removeFromTop(38).withWidth(260);
    const int octW=octaveRow.getWidth()/static_cast<int>(octaveButtons_.size());
    for(std::size_t i=0;i<octaveButtons_.size();++i) {
        auto cell=(i+1==octaveButtons_.size())?octaveRow:octaveRow.removeFromLeft(octW);
        octaveButtons_[i].setBounds(cell.reduced(1,3));
    }

    auto t=timing.reduced(18,46);
    const int knobW=t.getWidth()/2;
    gate_.setBounds(t.removeFromLeft(knobW).reduced(20,0));
    swing_.setBounds(t.reduced(20,0));
}

void ArpeggiatorPanel::paint(juce::Graphics& g) {
    auto shell=getLocalBounds().toFloat().reduced(.5f);
    g.setColour(Palette::panel());
    g.fillRoundedRectangle(shell,3.0f);
    g.setColour(Palette::borderSoft());
    g.drawRoundedRectangle(shell,3.0f,1.0f);

    auto area=getLocalBounds().reduced(18);
    auto heading=area.removeFromTop(58);
    text(g,"ARPEGGIATOR",heading.removeFromTop(28),17.0f,Palette::text());
    text(g,"Rhythmic note sequencing and shared master clock",
         heading.removeFromTop(18),9.0f,Palette::muted());

    auto master=area.removeFromTop(72);
    well(g,master);
    auto masterLabels=master.reduced(12,7).removeFromTop(12);
    text(g,"PERFORMANCE",masterLabels.removeFromLeft(190),7.0f,Palette::muted());
    text(g,"CLOCK",masterLabels.removeFromLeft(250),7.0f,Palette::muted(),juce::Justification::centred);
    text(g,"TEMPO",masterLabels,7.0f,Palette::muted(),juce::Justification::centredLeft);

    area.removeFromTop(14);
    auto monitor=area.removeFromTop(112);
    well(g,monitor);
    auto monitorInner=monitor.reduced(16,10);
    auto monitorHeader=monitorInner.removeFromTop(20);
    text(g,"LIVE SEQUENCE",monitorHeader.removeFromLeft(180),9.5f,Palette::secondary());
    ...
    text(g,clockText,monitorHeader,7.5f,Palette::muted(),juce::Justification::centredRight);

    std::array<int,32> preview{};
    int count=0;
    for(int note=0;note<128 && count<8;++note) {
        if(!held(runtime,note)) continue;
        for(int octave=0;octave<state.octaveSpan && count<static_cast<int>(preview.size());++octave) {
            const int generated=note+octave*12;
            if(generated<=127) preview[static_cast<std::size_t>(count++)]=generated;
        }
    }

    auto lane=monitorInner.reduced(8,10);
    ...
    if(count==0) {
        ...
        for(int i=0;i<count;++i) {
            ...
            text(g,noteName(note),
                 {static_cast<int>(x-step*.45f),static_cast<int>(cy+10),static_cast<int>(step*.9f),14},
                 active?8.2f:7.4f,active?Palette::text():Palette::muted(),juce::Justification::centred);
        }
    }
    ...
    auto p=pattern.reduced(16,12);
    text(g,"PATTERN",p.removeFromTop(20),10.0f,Palette::secondary());
    p.removeFromTop(10);
    text(g,"DIRECTION",p.removeFromTop(14),7.2f,Palette::muted());
    p.removeFromTop(42);
    text(g,"RATE / DIVISION",p.removeFromTop(14),7.2f,Palette::muted());
    p.removeFromTop(42);
    text(g,"OCTAVE SPAN",p.removeFromTop(14),7.2f,Palette::muted());

    auto t=timing.reduced(16,12);
    text(g,"TIMING",t.removeFromTop(20),10.0f,Palette::secondary());
    auto labels=t.removeFromBottom(24);
    const int half=labels.getWidth()/2;
    text(g,"GATE",labels.removeFromLeft(half),8.0f,Palette::muted(),juce::Justification::centred);
    text(g,"SWING",labels,8.0f,Palette::muted(),juce::Justification::centred);
}
