...
function getMessagesSidebarMarkup() {
  const filterMarkup = inboxFilters
    .map((filter) => {
      ...
      return `
        <a href="${filter.path}" class="inbox-filter ${appState.activeFilter === filter.label ? 'is-active' : ''}" data-inbox-filter="${filter.label}" data-inbox-path="${filter.path}" data-guide-id="inbox-sidebar-${escapeHtml(guideKey)}" data-guide-label="${escapeHtml(filter.label)}" data-guide-role="inbox-sidebar-link">
...
function bindSharedEvents(scope = inboxRoot) {
  ...
  scope.querySelectorAll('[data-inbox-filter]').forEach((control) => {
    control.addEventListener('click', (event) => {
      // Keep SPA navigation when click delivery works. The real href remains a
      // native WebKit/iOS fallback when custom click activation is lost.
      event.preventDefault()
      const nextFilter = control.dataset.inboxFilter || 'Messages'
      if (nextFilter !== 'Messages' && activeTypingThreadId) {
        clearTypingForThread(activeTypingThreadId)
      }
      navigateInbox(control.dataset.inboxPath || control.getAttribute('href') || ROUTES.inboxMessages)
    })
  })
