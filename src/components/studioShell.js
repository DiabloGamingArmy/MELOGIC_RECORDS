...
const STUDIO_APPS = [
  ...
  { key: 'inkora', label: 'Inkora', href: ROUTES.studioInkora, icon: 'inkora', iconFile: 'Inkora' },
...
export function studioSidebar({ active = 'projects' } = {}) {
  ...
  const apps = STUDIO_APPS.map((item) => {
    ...
    return `<a class="studio-sidebar-link studio-sidebar-app ${activeKeys.includes(active) ? 'is-active' : ''}" href="${item.href}" ${activeKeys.includes(active) ? 'aria-current="page"' : ''}>${appIcon(item)}<span>${item.label}</span></a>`
  }).join('')
  ...
  return `<aside class="studio-sidebar">
    ...
    <nav class="studio-sidebar-nav" aria-label="Studio sections">
      <a class="studio-sidebar-link studio-sidebar-home ${is('hub')}" href="${ROUTES.studio}" ${active === 'hub' ? 'aria-current="page"' : ''}>Studio Home</a>
      ...
      <div class="studio-sidebar-group studio-sidebar-group--resources">
        ...
        <a class="studio-sidebar-link ${is('live')}" href="${ROUTES.studioLive}">Live Studio</a>
