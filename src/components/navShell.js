  ...
  return `
    <div class="mobile-app-shell">
      <header class="mobile-app-header" ${isCommunity ? 'data-community-mobile-header' : ''}>
        ...
        <div class="mobile-app-actions">${isCommunity ? `<a class="mobile-icon-button" href="${ROUTES.community}?search=1" aria-label="Search Community">⌕</a><a class="mobile-icon-button" href="${ROUTES.inboxContent}" aria-label="Notifications">♢</a><a class="mobile-icon-button mobile-create-button" href="${ROUTES.communityCreate}" aria-label="Create post">＋</a>` : ''}</div>
      </header>
      <nav class="mobile-bottom-nav" aria-label="Mobile primary navigation" style="grid-template-columns:repeat(6,minmax(0,1fr))">
