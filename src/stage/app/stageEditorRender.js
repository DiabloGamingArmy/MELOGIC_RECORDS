...
function renderViewport() {
  ...
  const toolButtons = editorToolModes.map((tool) => `<button type="button" data-tool-mode="${tool.key}" class="stage-tool-mode ${state.editorToolMode === tool.key ? 'is-active' : ''}" aria-pressed="${state.editorToolMode === tool.key}" title="${tool.label} (${shortcutByTool[tool.key] || ''})">${tool.label}</button>`).join('')
