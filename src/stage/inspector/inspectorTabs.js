...
export function selectedEditorObjectMarkup() {
  ...
  const blueprintNote = selected.type === 'blueprint-directional-note'
    ? section('Blueprint Directional Note', `<label class="vertix-notes-field"><span>Text</span><textarea data-vertix-transform-field="noteText" placeholder="Directional Note">${escapeHtml(selected.metadata?.text || '')}</textarea></label><p class="stage-safety-note">Blueprint notes are camera-facing technical annotations.</p>`)
    : ''
  ...
  return `<div class="vertix-universal-properties" data-guide-id="stagemaker-entity-${escapeAttr(selected.id)}" data-guide-label="${escapeAttr(selected.label || selected.id)}" data-guide-role="stage-entity" data-guide-entity-id="${escapeAttr(selected.id)}">${header}${transform}${display}${blueprintNote}${layer}${stageComponentSection(selected)}${sourceSection(selected)}${customPropertiesSection(selected)}</div>`
}
