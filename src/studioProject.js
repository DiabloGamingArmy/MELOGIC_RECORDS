...
const renderTrackRow = (t) => `<div class="studio-track-stack"><article class="studio-track-row ${selectedTrackId === t.id ? 'is-selected' : ''}" data-track-row="${t.id}"><span class="studio-track-color" style="background:${t.color}"></span><input class="studio-track-select" type="checkbox" aria-label="Select ${t.name} for merge" data-track-select="${t.id}" ${selectedMergeTrackIds.has(t.id) ? 'checked' : ''}/><span class="studio-track-type-icon" aria-hidden="true">${trackTypeIcon(t.type)}</span><strong class="studio-track-name">${t.name}</strong><button class="studio-track-control" data-track-mute="${t.id}" aria-label="Mute ${t.name}" data-tooltip="Mute">${icon('mute')}</button><button class="studio-track-control" data-track-solo="${t.id}" aria-label="Solo ${t.name}" data-tooltip="Solo">${icon('solo')}</button><button class="studio-track-control" data-track-record="${t.id}" aria-label="Record arm ${t.name}" data-tooltip="Record arm">${icon('record')}</button><button class="studio-track-control" data-track-automation="${t.id}" aria-label="Show automation for ${t.name}" data-tooltip="Automation">${icon('automation')}</button><input type="range" min="0" max="100" value="72" class="studio-track-volume" aria-label="${t.name} volume"/><button class="studio-pan-knob" aria-label="${t.name} pan" data-tooltip="Pan"></button><button class="studio-track-control studio-track-more" data-track-options="${t.id}" aria-label="Track options" data-tooltip="Track options">${icon('more')}</button></article>${automationOpenTrackIds.has(t.id) ? '<div class="studio-automation-lane"><span>Automation</span><button>Volume</button><button>Pan</button><button>Filter</button><button>Add</button></div>' : ''}<div class="studio-track-menu" ${openTrackMenuId === t.id ? '' : 'hidden'}><button>Rename</button><button>Duplicate</button><button disabled>Delete</button><button>Freeze Track</button><button>Freeze All Tracks</button><button>Import MIDI File</button><button>Export Track</button><button>Track Color</button><button ${selectedMergeTrackIds.size < 2 ? 'disabled' : ''}>Merge Tracks</button></div></div>`
