...
function renderBottomPanel(panel){ const t={loops:['Loop Browser','Loops and samples will appear here.'],fx:['Effects','Track effects and processors will appear here.'],mixer:['Mixer','Channel strips and routing will appear here.'],collab:['Collaboration','Project presence, comments, and invites will appear here.']}[panel]||['Panel','']; return `<section class="studio-bottom-panel"><header class="studio-bottom-panel-header"><strong>${t[0]}</strong><button data-close-bottom-panel aria-label="Close panel">✕</button></header><div class="studio-bottom-panel-body"><p>${t[1]}</p></div></section>` }
...
function bindEditorEvents() {
  ...
  app.querySelectorAll('[data-track-volume]').forEach((el) => el.addEventListener('input', (e) => { e.stopPropagation(); const t = getTrack(el.dataset.trackVolume); if (!t) return; t.volume = Number(el.value); }))
  ...
  app.querySelector('[data-transport-play]')?.addEventListener('click', (e)=>{ e.stopPropagation(); isPlaying = !isPlaying; renderEditor() })
  app.querySelector('[data-transport-stop]')?.addEventListener('click', (e)=>{ e.stopPropagation(); isPlaying = false; renderEditor() })
...
if (!window.__studioSpaceKeyBound) { window.__studioSpaceKeyBound = true; document.addEventListener('keydown', (event) => { if (event.code === 'Space' && !event.target.matches('input, textarea, [contenteditable="true"]')) { event.preventDefault(); isPlaying = !isPlaying; renderEditor() } }) }
