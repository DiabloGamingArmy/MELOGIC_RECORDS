...
const state = { user: null, loadingProjects: false, projectsError: '', projects: [], recentProjects: [], projectId: '', editorLoading: false, editorError: '', projectLoadStatus: 'loading', editorProject: null, createError: '', selectedStageType: 'Blank Stage', activeEditorMode: 'builder', activeRailSection: 'object', activeLibraryCategory: 'band-backline', selectedEditorObject: 'stage-deck', editorObjectTransforms: {}, snapEnabled: true, snapInterval: 1, measureMode: false, showExportPreview: false, showStageGlobalHeader: false, stageAppMenuOpen: false, showViewportDiagnostics: false, viewportMode: localStorage.getItem('stageViewportMode') || 'perspective3d', paneSizes: { library: Number(localStorage.getItem('stagePaneLibrary')) || 236, right: Number(localStorage.getItem('stagePaneRight')) || 286, bottom: Number(localStorage.getItem('stagePaneBottom')) || 190 } }
...
function renderLeftPanelBySection(title, stamp) {
  ...
  const map = {
    ...
    settings: `${base}<section class="stage-config-panel"><h3>Settings</h3><p>Units, default snap/view, diagnostics toggle, autosave placeholder.</p><label class="stage-editor-check"><input data-toggle-viewport-diagnostics type="checkbox" ${state.showViewportDiagnostics ? 'checked' : ''} /> Show viewport diagnostics</label></section>`,
