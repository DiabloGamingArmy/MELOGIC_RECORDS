...
function initStageEditorViewport() {
  ...
  cleanupStageViewport = mountStageThreeViewport(container, {
    ...
    getSelectedObjectKey: () => state.selectedEditorObject,
    onSelectObject: (key) => {
...
function renderApp() { app.innerHTML = `${navShell({ currentPage: 'stage' })}${state.projectId ? renderEditor() : renderDashboard()}`; initShellChrome(); bindEvents(); initStageEditorViewport() }
