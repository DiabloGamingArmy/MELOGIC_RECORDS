...
const editorLibraryCategories = [
  { key: 'band-backline', label: 'Band / Backline' }, { key: 'lighting', label: 'Lighting' }, { key: 'rigging', label: 'Rigging' }, { key: 'audio', label: 'Audio' },
  { key: 'video', label: 'Video' }, { key: 'venue', label: 'Venue' }, { key: 'patching', label: 'Patching' }, { key: 'cases', label: 'Cases' }
]
...
const state = { user: null, loadingProjects: false, projectsError: '', projects: [], recentProjects: [], projectId: '', editorLoading: false, editorError: '', editorProject: null, createError: '', selectedStageType: 'Blank Stage', activeEditorMode: 'builder', activeLibraryCategory: 'band-backline', selectedEditorObject: 'stage-deck' }
...
function renderEditor() { /* unchanged behavior */
  ...
  const modeBody = state.activeEditorMode === 'stage-plot' ? '<section class="stage-editor-mode-panel"><h4>2D Stage Plot</h4><div class="stage-mode-large"></div></section>'
    : state.activeEditorMode === 'input-list' ? '<section class="stage-editor-table-panel is-large"><h4>Auto-Generated Input List</h4><table><thead><tr><th>Ch</th><th>Source</th><th>Mic/DI</th><th>Stand</th><th>Notes</th></tr></thead><tbody><tr><td>1</td><td>Kick In</td><td>Beta 91A</td><td>Short</td><td>USC</td></tr><tr><td>2</td><td>Kick Out</td><td>Beta 52</td><td>Short</td><td>USC</td></tr><tr><td>8</td><td>Bass DI</td><td>DI</td><td>N/A</td><td>USC</td></tr><tr><td>12</td><td>Lead Vocal</td><td>Wireless</td><td>N/A</td><td>DSC</td></tr><tr><td>21</td><td>Playback L</td><td>DI</td><td>N/A</td><td>SR</td></tr><tr><td>22</td><td>Playback R</td><td>DI</td><td>N/A</td><td>SL</td></tr></tbody></table></section>'
  ...
  return `<main class="stage-editor-app"><section class="stage-editor-menubar"><div class="stage-editor-menu-left"><strong>MELOGIC STAGE</strong><button type="button">File</button><button type="button">Edit</button><button type="button">View</button><button type="button">Share</button></div><div class="stage-editor-project-title"><h2>Project: ${title}</h2><p>Date/Version ${stamp} | v${state.editorProject?.version || 1}</p></div><div class="stage-editor-menu-actions"><button type="button" aria-disabled="true">Share ▾</button><button type="button" aria-disabled="true">?</button><button type="button" aria-disabled="true">⋯</button><button type="button" class="is-send" aria-disabled="true">Send Stage Plan</button></div></section><section class="stage-editor-body"><nav class="stage-editor-rail" aria-label="Editor tools">${editorRailItems.map((item, i) => `<button type="button" class="${i === 1 ? 'is-active' : ''}">${item}</button>`).join('')}<a class="stage-back-link" href="${ROUTES.stage}">Exit</a></nav><aside class="stage-editor-library"><header><h3>OBJECT LIBRARY</h3></header><div class="stage-editor-library-tools"><input aria-label="Search object library" placeholder="Search" /><button type="button" aria-disabled="true">⌯</button></div><label class="stage-editor-check"><input type="checkbox" /> Drag and drop categories</label><h4>BAND / BACKLINE</h4><div class="stage-object-grid">${editorLibraryCategories.map((c) => `<button class="stage-object-tile ${state.activeLibraryCategory === c.key ? 'is-active' : ''}" data-library-category="${c.key}" type="button">${c.label}</button>`).join('')}</div><h4>BASE STAGE TYPES</h4><div class="stage-base-stage-grid">${baseStageTypes.map((s) => `<button class="stage-base-stage-card" type="button" aria-disabled="true">${s}</button>`).join('')}</div></aside><section class="stage-editor-workspace"><div class="stage-editor-viewport"><header class="stage-editor-viewport-toolbar"><span>${state.editorProject?.stageType || 'Blank Stage'}</span><button type="button" aria-disabled="true">Beam Preview</button><button type="button" aria-disabled="true">Grid</button><button type="button" aria-disabled="true">Snap</button></header><div class="stage-editor-canvas" aria-label="Mock stage workspace" role="img"><button class="stage-canvas-object stage-canvas-deck ${state.selectedEditorObject === 'stage-deck' ? 'is-selected' : ''}" data-editor-object="stage-deck" aria-label="Select Stage Deck"></button><button class="stage-canvas-object stage-canvas-riser ${state.selectedEditorObject === 'drum-riser' ? 'is-selected' : ''}" data-editor-object="drum-riser" aria-label="Select Drum Riser"></button><button class="stage-canvas-object stage-canvas-truss ${state.selectedEditorObject === 'truss-a' ? 'is-selected' : ''}" data-editor-object="truss-a" aria-label="Select Truss A"></button><button class="stage-canvas-object stage-canvas-light ${state.selectedEditorObject === 'moving-head' ? 'is-selected' : ''}" data-editor-object="moving-head" aria-label="Select Moving Head"></button><button class="stage-canvas-object stage-canvas-camera ${state.selectedEditorObject === 'camera-1' ? 'is-selected' : ''}" data-editor-object="camera-1" aria-label="Select Camera 1"></button><span class="stage-canvas-tag stage-canvas-tag-a" aria-hidden="true">32' x 24' Stage Deck</span><span class="stage-canvas-tag stage-canvas-tag-b" aria-hidden="true">Drum Riser 8' x 8'</span><span class="stage-canvas-tag stage-canvas-tag-c" aria-hidden="true">Ground supported trusses</span><span class="stage-canvas-label l1" aria-hidden="true">Stage Left</span><span class="stage-canvas-label l2" aria-hidden="true">Stage Right</span><span class="stage-canvas-label l3" aria-hidden="true">DSC</span><span class="stage-canvas-label l4" aria-hidden="true">USC</span><span class="stage-canvas-label l5" aria-hidden="true">FOH</span></div></div></section><aside class="stage-editor-right"><section class="stage-config-panel"><h3>Selected: ${selected.label}</h3><p>Type: ${selected.type}</p>${detailRows}${selected.key === 'moving-head' ? '<p>Fixture: Moving Head</p><p>DMX Address: 001/U1</p><p>Mode: 24ch</p><p>Color: Cyan</p><p>Beam Angle: 15-45°</p><input type="range" disabled />' : ''}</section><section class="stage-ai-panel"><h3>AI Assistant</h3><textarea aria-label="AI prompt" placeholder="Build me a 24x16 stage for a 5-piece metalcore band with tracks and lighting."></textarea><button type="button" aria-disabled="true">Generate</button></section><section class="stage-config-panel"><h3>Project Info</h3><p>${title}</p><p>${state.editorProject?.stageType || 'Blank Stage'}</p><p>Category: ${editorLibraryCategories.find((c) => c.key === state.activeLibraryCategory)?.label || 'Band / Backline'}</p><p>Status: Foundation Preview</p></section></aside><section class="stage-editor-bottom"><div class="stage-editor-mode-tabs">${editorModes.map((m) => `<button class="stage-editor-mode-tab ${state.activeEditorMode === m.key ? 'is-active' : ''}" data-editor-mode="${m.key}" type="button" aria-selected="${state.activeEditorMode === m.key}">${m.label}</button>`).join('')}</div>${modeBody}</section></section></main>`
}
...
function bindEvents() {
  ...
  app.querySelectorAll('[data-library-category]').forEach((el) => el.addEventListener('click', () => { state.activeLibraryCategory = el.dataset.libraryCategory || 'band-backline'; renderApp() }))
