...
// melogic-mobile-unified-runtime-v4
function registerStreamingMobileRuntime() {
  if (!isMobileSpaRuntime()) return
  ...
  registerMobileRuntimeView('streaming', {
    ...
    async deactivate({ instance }) {
      mobileStreamingRuntimeActive = false
      document.body.classList.remove('is-streaming-page')
      musicLiveControlsCleanup?.()
      musicMonitorVisualizerCleanup?.()
      if (musicMonitorControlsTimer) {
        window.clearTimeout(musicMonitorControlsTimer)
        musicMonitorControlsTimer = 0
      }
    ...
    async unmount({ instance }) {
      mobileStreamingRuntimeActive = false
      instance.fragment = null
      try { cleanupHlsPlayback() } catch {}
      try { cleanupNativeMediaSourcePipeline() } catch {}
      try { await releaseLiveListenerPresence() } catch {}
      try { await state.listenerRoom?.disconnect?.() } catch {}
...
function renderAppShell(content) {
  // melogic-mobile-unified-runtime-v4
  if (isMobileSpaRuntime() && !mobileStreamingRuntimeActive) {
    mobileStreamingRuntimeDirty = true
    return
  }
