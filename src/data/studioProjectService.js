...
const FIRESTORE_PROJECT_SOFT_LIMIT_BYTES = 900 * 1024
...
function compactStudioEditorStateForFirestore(editorState) {
  const safe = sanitizeForFirestore(editorState)
  if (!safe || typeof safe !== 'object') return safe

  // Never persist binary/runtime payloads or transient URLs into Firestore.
  // Audio itself is already uploaded to Firebase Storage; Firestore stores only
  // the durable storagePath plus compact editing metadata.
  const visit = (value, key = '') => {
    ...
    for (const [childKey,childValue] of Object.entries(value)) {
      ...
      if (['blob','file','audiobuffer','arraybuffer','rawaudio','audiodata','filedata','binarydata'].includes(lower)) continue
      if ((lower==='peaks' || lower==='waveform') && Array.isArray(childValue)) {
        out[childKey]=childValue.slice(0,1200).map((item)=>visit(item,childKey))
        continue
      }
...
export async function saveStudioProjectEditorState(projectId, editorState) {
  ...
  const payloadBytes = utf8Size(safeEditorState)
  if (payloadBytes > FIRESTORE_PROJECT_SOFT_LIMIT_BYTES) {
    console.warn('[studioProject] editorState remains unusually large after compaction', {
      bytes: payloadBytes,
      softLimitBytes: FIRESTORE_PROJECT_SOFT_LIMIT_BYTES
    })
  }
