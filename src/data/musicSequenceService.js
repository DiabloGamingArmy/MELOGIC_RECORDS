...
function normalizeAsset(raw = {}, id = '') {
  return {
    ...
    defaultCrossfadeMs: Math.max(0, toNumber(raw.defaultCrossfadeMs)),
...
function normalizeSequenceItem(raw = {}, id = '') {
  return {
    ...
    videoPathSnapshot: cleanString(raw.videoPathSnapshot, 500),
    videoURLSnapshot: cleanString(raw.videoURLSnapshot, 1400),
    durationMs: Math.max(0, toNumber(raw.durationMs)),
...
export async function createSequenceAssetShell(uid = '', data = {}) {
  ...
  const payload = {
    ...
    defaultCrossfadeMs: Math.max(0, Math.round(Number(data.defaultCrossfadeMs) || 0)),
    cueInMs: Math.max(0, Math.round(Number(data.cueInMs) || 0)),
    cueOutMs: Math.max(0, Math.round(Number(data.cueOutMs) || 0)),
    gainDb: Number.isFinite(Number(data.gainDb)) ? Number(data.gainDb) : 0,
    loudnessLUFS: null,
...
export async function addAssetToSequence(uid = '', sequenceId = '', asset = {}, sequence = {}) {
  ...
  const item = {
    ...
    videoPathSnapshot: asset.videoPath || '',
    videoURLSnapshot: asset.videoURL || '',
    durationMs: asset.durationMs || 0,
    startOffsetMs: 0,
    endOffsetMs: 0,
    introMs: 0,
    outroMs: 0,
    cueInMs: asset.cueInMs || 0,
    cueOutMs: asset.cueOutMs || 0,
    fadeInMs: asset.defaultFadeInMs || sequence.defaultFadeInMs || 0,
    fadeOutMs: asset.defaultFadeOutMs || sequence.defaultFadeOutMs || 0,
    crossfadeMs: asset.defaultCrossfadeMs || sequence.defaultCrossfadeMs || 0,
    gainDb: asset.gainDb || 0,
    playbackRate: 1,
    enabled: true,
    isPinned: false,
    notes: '',
