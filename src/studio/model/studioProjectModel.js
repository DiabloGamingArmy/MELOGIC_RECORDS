...
function normalizeStretchEdit(region = {}, sourceDurationSeconds = 0, visibleDurationSeconds = 0) {
  ...
  return {
    ...
    algorithm: String(source.algorithm || 'signalsmith_wasm_time_stretch_v1'),
  ...
  return {
    ...
    audioClip: type === 'audio' && region.audioClip && typeof region.audioClip === 'object'
      ? {
        ...
        fileName: region.audioClip.fileName || region.fileName || '',
        fileSizeBytes: region.audioClip.fileSizeBytes == null ? null : num(region.audioClip.fileSizeBytes, 0),
        sampleRate: region.audioClip.sampleRate == null ? null : num(region.audioClip.sampleRate, 0),
        bitDepth: region.audioClip.bitDepth == null ? null : num(region.audioClip.bitDepth, 0),
        bitDepthSource: region.audioClip.bitDepthSource || region.bitDepthSource || 'unknown',
        channelCount: region.audioClip.channelCount == null ? null : num(region.audioClip.channelCount, 0),
