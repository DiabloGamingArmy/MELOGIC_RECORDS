...
function validateForSubmission(release = {}, tracks = []) {
  ...
  if (!release.coverArtPath && !release.coverArtURL) problems.push('Cover artwork is required.')
...
const saveMusicReleaseDraft = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  const rawTracks = Array.isArray(request.data?.tracks) ? request.data.tracks.slice(0, 50) : []
...
const submitMusicRelease = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  if (!releaseSnap.exists) throw new HttpsError('not-found', 'Release draft not found.')
  ...
  const trackDocs = await releaseTracks(releaseId)
  const tracks = trackDocs.map((snap) => ({ id: snap.id, ...(snap.data() || {}) }))
  const problems = validateForSubmission(release, tracks)
  if (problems.length) {
    throw new HttpsError('failed-precondition', problems.join(' '), { problems })
  }
  await assertUniqueIdentifiers({ releaseId, upc: release.upc, tracks })
  const now = admin.firestore.FieldValue.serverTimestamp()
  const nextClaims = [
  ...
  await db().runTransaction(async (transaction) => {
    ...
    trackDocs.forEach((trackSnap) => transaction.set(trackSnap.ref, {
      status: 'submitted',
      visibility: 'private',
      updatedAt: now
    }, { merge: true }))
  })
  await writeAccountEvent(db(), requester.uid, {
    type: 'music_release_submitted',
    severity: 'info',
    title: 'Music release submitted',
    message: `${release.title || 'Your release'} was submitted for Melogic Streaming review.`,
...
const reviewMusicRelease = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  const release = releaseSnap.data() || {}
  if (release.status !== 'submitted') throw new HttpsError('failed-precondition', 'Only submitted releases can be reviewed.')
  const trackDocs = await releaseTracks(releaseId)
  const tracks = trackDocs.map((snap) => ({ id: snap.id, ...(snap.data() || {}) }))
  if (decision === 'approve') {
    const problems = validateForSubmission(release, tracks)
  ...
  trackDocs.forEach((trackSnap) => batch.set(trackSnap.ref, {
    status: nextStatus,
    visibility: nextVisibility,
    reviewedAt: now,
    reviewedBy: reviewer.uid,
  ...
  const auditLogId = await writeAdminAuditLog({
    ...
    reason,
    before: { status: release.status, visibility: release.visibility, title: release.title },
    after: { status: nextStatus, visibility: nextVisibility, title: release.title },
    metadata: { trackCount: trackDocs.length, sourceDistributor: release.sourceDistributor || '' }
  })
  await writeAccountEvent(db(), release.artistUid, {
    ...
    message: decision === 'approve'
      ? `${release.title || 'Your release'} is now available in Melogic Streaming.`
      : `${release.title || 'Your release'} was returned: ${reason}`,
    actorUid: reviewer.uid,
    actorType: 'admin',
    source: 'music-distribution',
