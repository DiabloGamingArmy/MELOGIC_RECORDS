...
const recordCommunityStoryView = onCall({ timeoutSeconds: 60, memory: '256MiB' }, async (request) => {
  ...
  const result = await firestore.runTransaction(async (tx) => {
    ...
    if (!storyViewDecision({ authorUid: story.authorUid, viewerUid }).incremented) {
    ...
    if (!storyViewDecision({ authorUid: story.authorUid, viewerUid, alreadyViewed: viewSnap.exists }).incremented) {
      return { viewCount: currentCount, incremented: false }
    }
    tx.set(viewRef, {
      storyId: storyRef.id,
