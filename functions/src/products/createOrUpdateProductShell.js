...
function baseProductPayload({ product = {}, productId = '', uid = '', existing = null } = {}) {
  ...
  const payload = {
    ...
    usageLicenseVersion: Math.max(0, Math.round(Number(product.usageLicenseVersion || existing?.usageLicenseVersion || 0) || 0)),
    usageLicensePath: normalizePath(product.usageLicensePath || existing?.usageLicensePath || ''),
    usageLicenseKey: cleanString(product.usageLicenseKey || existing?.usageLicenseKey || '', 80),
