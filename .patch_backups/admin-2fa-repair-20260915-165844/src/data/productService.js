...
async function requireAdminStepUp(name) {
  const guard=window.__melogicAdminRequireStepUp
  if (typeof guard !== 'function') {
    const error=new Error('Admin 2FA security controller is unavailable. Refresh the Admin Panel.')
    error.code='admin/step-up-controller-unavailable'
    throw error
  }
  const ok=await guard(name)
  if (!ok) {
    const error=new Error('Admin action cancelled before 2FA verification.')
    error.code='admin/step-up-cancelled'
    throw error
  }
}

function adminProtectedCallable(name) {
  const callable=httpsCallable(functions,name)
  return async (payload) => {
    await requireAdminStepUp(name)
...
export function normalizeProduct(productId, rawProduct = {}, media = {}) {
  ...
  return {
    ...
    usageLicenseVersion: Number(rawProduct.usageLicenseVersion || 0) || 0,
    usageLicensePath: rawProduct.usageLicensePath || '',
    usageLicenseKey: rawProduct.usageLicenseKey || '',
...
export function buildProductPayload(input = {}, user = null) {
  ...
  return {
    ...
    usageLicenseVersion: Number(input.usageLicenseVersion || 0) || 0,
    usageLicensePath: input.usageLicensePath || '',
    usageLicenseKey: input.usageLicenseKey || '',
...
const FIRESTORE_PRODUCT_CLIENT_ALLOWED_KEYS = new Set([
  ...
  'title', 'shortDescription', 'description', 'version', 'usageLicense', 'usageLicenseVersion', 'usageLicensePath', 'usageLicenseKey', 'productType', 'productKind', 'previewMode', 'distributionMode',
...
export async function saveProductManifest({ productId, draft = {}, uploadedFiles = [] } = {}) {
  ...
  const manifest = {
    ...
    usageLicense: draft.usageLicense || '',
    usageLicenseVersion: Number(draft.usageLicenseVersion || 0) || 0,
    usageLicensePath: draft.usageLicensePath || '',
    usageLicenseKey: draft.usageLicenseKey || '',
    assetSummary: {
...
export async function reviewProductDecision({ productId = '', decision = '', reason = '', notes = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('reviewProductDecision')
...
export async function setAdminUserRole({ uid = '', role = '', active = true, reason = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('setAdminUserRole')
...
export async function updateAdminAccountPermissions(payload = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('updateAdminAccountPermissions')
  const result = await callable(payload)
  return result?.data || { ok: false, explicit: {}, effective: {} }
}
...
export async function grantAdminProducts({ uid = '', productIds = [] } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('grantAdminProducts')
...
export async function addAdminUserNote({ uid = '', note = '', severity = 'info', category = 'account' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('addAdminUserNote')
...
export async function setUserSuspension({ uid = '', suspended = true, reason = '', duration = 'indefinite', note = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('setUserSuspension')
...
export async function updateReportDecision({ reportId = '', action = '', reason = '', notes = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('updateReportDecision')
...
export async function repairAdminCheckoutOrder({ orderId = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('repairAdminCheckoutOrder')
  const result = await callable({ orderId })
  return result?.data || { ok: false }
}
...
export async function updateAdminSettings({ section = '', values = {}, reason = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('updateAdminSettings')
...
export async function sendAdminEmail({ to = '', subject = '', body = '', category = 'support', templateType = 'support', ctaLabel = '', ctaUrl = '', relatedUid = '', relatedProductId = '', relatedOrderId = '', relatedReportId = '', cc = '', replyTo = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('sendAdminEmail')
...
export async function sendAdminAuthEmail({ uid = '', type = '', to = '', subject = '', body = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('sendAdminAuthEmail')
...
export async function forcePasswordReset({ uid = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('forcePasswordReset')
  const result = await callable({ uid })
  return result?.data || { ok: false }
}
...
export async function setTemporaryPassword({ uid = '', password = '', confirmation = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('setTemporaryPassword')
  const result = await callable({ uid, password, confirmation })
  return result?.data || { ok: false }
}

export async function unverifyUserEmail({ uid = '', confirmation = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('unverifyUserEmail')
...
export async function revokeRecoveryCodes({ uid = '', confirmation = '' } = {}) {
  if (!functions) throw new Error('Functions are not configured.')
  const callable = adminProtectedCallable('revokeRecoveryCodes')
  const result = await callable({ uid, confirmation })
  return result?.data || { ok: false }
}
