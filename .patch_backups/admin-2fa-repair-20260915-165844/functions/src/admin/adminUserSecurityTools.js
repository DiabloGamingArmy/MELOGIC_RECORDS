...
const unverifyUserEmail = onCall({ timeoutSeconds: 60, memory: '256MiB' }, async (request) => {
  const actor = await requireAdminActionSecurity(request, 'roleManage')
  await assertOwnerOnly(actor)
  const uid = safeUid(request.data?.uid || '')
  if (uid === actor.uid) throw new HttpsError('failed-precondition', 'You cannot unverify your own admin email.')
  const user = await admin.auth().getUser(uid)
  if (!user.email) throw new HttpsError('failed-precondition', 'This account does not have an email address.')
  if (!user.emailVerified) return { ok: true, emailVerified: false, alreadyUnverified: true }
  ...
  await writeAdminAuditLog({
    ...
    actorEmail: actor.email,
    actorRole: actor.adminRole,
    action: 'admin_email_unverified',
    targetType: 'user',
    targetId: uid,
    targetPath: `users/${uid}`,
    reason: 'Email verification revoked by owner'
  })

  await writeAccountEvent(admin.firestore(), uid, {
    type: 'email_verification_revoked',
    severity: 'warning',
    title: 'Email verification removed',
    message: 'Melogic Records Support marked your account email as unverified. Verify your email again to restore verified-email access.',
    actorUid: actor.uid,
    actorType: 'admin',
    source: 'admin-security-tools',
    path: '/account/security'
