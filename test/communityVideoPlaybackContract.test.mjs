...
test('Community feed video never delegates playback to raw native controls', () => {
  ...
  assert.match(videoMarkup, /\sloop(?:\s|>)/)
...
test('feed video coordinator keeps one playback authority and guards lifecycle handoffs', () => {
  ...
  assert.match(playback, /video\.loop = true/)
  assert.match(playback, /let communityFeedMuted = true/)
  assert.match(playback, /communityFeedMuted = Boolean\(muted\)/)
  assert.match(playback, /targets\.forEach\(\(candidate\) => applyGlobalMutedState\(candidate\)\)/)
  assert.match(playback, /applyGlobalMutedState\(video\)/)
